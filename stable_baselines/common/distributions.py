...
class CategoricalProbabilityDistributionType(ProbabilityDistributionType):
  ...
  def __init__(self, n_cat, is_DQN=False):
    """
  ...
  def proba_distribution_from_latent(self, pi_latent_vector, vf_latent_vector, init_scale=1.0,
    ...
    if self.is_DQN:
      return self.proba_distribution_from_flat(q_values), q_values, q_values
...
class MultiCategoricalProbabilityDistributionType(ProbabilityDistributionType):
  ...
  def proba_distribution_from_latent(self, pi_latent_vector, vf_latent_vector, init_scale=1.0,
                                     init_bias=0.0):
    pdparam = linear(pi_latent_vector, 'pi', sum(self.n_vec), init_scale=init_scale,
                     init_bias=init_bias)
    q_values = linear(vf_latent_vector, 'q', sum(self.n_vec), init_scale=init_scale,
                      init_bias=init_bias)
    return self.proba_distribution_from_flat(pdparam), pdparam, q_values
...
class CategoricalProbabilityDistribution(ProbabilityDistribution):
  ...
  def sample(self):
    return tf.squeeze(tf.multinomial(self.logits, 1), [-1])
...
class BernoulliProbabilityDistribution(ProbabilityDistribution):
  ...
  def neglogp(self, x):
    return tf.reduce_sum(
        tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=tf.to_float(x)), axis=-1)
  ...
  def entropy(self):
    return tf.reduce_sum(
        tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=self.probabilities),
        axis=-1)
...
def make_proba_dist_type(ac_space, is_DQN=False):
  """
  ...
  if isinstance(ac_space, spaces.Box):
    ...
    return CategoricalProbabilityDistributionType(ac_space.n, is_DQN)
