...
class GoalDQN(SimpleRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=5e-4, *, exploration_fraction=0.1,
        ...
        super(GoalDQN, self).__init__(policy=policy, env=env, verbose=verbose, requires_vec_env=False)
    ...
    def _setup_model(self):
        with SetVerbosity(self.verbose):
            assert isinstance(self.action_space, gym.spaces.Discrete), \
                "Error: GoalDQN only supports gym.spaces.Discrete action space."
            self.graph = tf.Graph()
            with self.graph.as_default():
                self.sess = tf_util.make_session(graph=self.graph)
                n_actions = self.action_space.n
                ...
                with tf.variable_scope("deepq"):
                    ...
                    # policy function
                    policy = self.policy(self.sess, self.observation_space, self.action_space, n_env=1, n_steps=1,
                                         n_batch=None, is_DQN=True, goal_space=self.goal_space)
                    ...
                    # create target q network evaluation
                    with tf.variable_scope("target_q_func", reuse=False):
                        target_policy = self.policy(self.sess, self.observation_space, self.action_space, 1, 1, None,
                                                    reuse=False, is_DQN=True, goal_space=self.goal_space)

                    # variables for each
                    q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                                    scope=tf.get_variable_scope().name + "/model")
                    target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                                           scope=tf.get_variable_scope().name + "/target_q_func")
                    ...
                    with tf.variable_scope("double_q", reuse=True,
                        ...
                        double_policy = self.policy(self.sess, self.observation_space, self.action_space, 1, 1, None,
                                                    reuse=True,
                ...
                with tf.variable_scope("loss"):
                    ...
                    if self.double_q:
                        ...
                        q_tp1_best = tf.reduce_sum(
                            target_policy.q_values * tf.one_hot(q_tp1_best_using_online_net, n_actions), axis=1)
                    else:
                        q_tp1_best = tf.reduce_max(target_policy.q_values, axis=1)
                    q_tp1_best_masked = (1.0 - done_mask_ph) * q_tp1_best
                    targets = rew_t_ph + self.gamma * q_tp1_best_masked
    ...
    def _setup_new_task(self, total_timesteps):
        ...
        items = [("observations0", self.observation_space.shape), \
                 ...
                 ("observations1", self.observation_space.shape), \
                 ("terminals1", (1,)), \
                 ("achieved_goal", env.observation_space.spaces['achieved_goal'].shape), \
                 ("desired_goal", env.observation_space.spaces['desired_goal'].shape)]
    ...
    def _get_action_for_single_obs(self, obs):
        """Called during training loop to get online action (with exploration)"""
        feed_dict = {
            self.model.obs_ph: np.array(obs["observation"])[np.newaxis],  # adds to new axis to convert to batch size of 1
    ...
    def _process_experience(self, obs, action, rew, new_obs, done):
        ...
        if self.task_step > self.learning_starts:
            if self.task_step % self.train_freq == 0:
                obses_t, actions, rewards, obses_tp1, dones, achieved_g, desired_g = self.replay_buffer.sample(self.batch_size)
                rewards = np.squeeze(rewards, 1)
                dones = np.squeeze(dones, 1)
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True, goal=None):
        return super(GoalDQN, self).predict(observation, state=state, mask=mask, deterministic=deterministic, goal=goal)
...
class GoalDQN(SimpleRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("deepq"):
                    ...
                    # policy function
                    policy = self.policy(self.sess, self.observation_space, self.action_space, n_env=1, n_steps=1,
                                         n_batch=None, is_DQN=True, self.goal_space)
                    deterministic_actions = tf.argmax(policy.q_values, axis=1)

                    batch_size = tf.shape(policy.obs_ph)[0]
                    random_actions = tf.random_uniform(tf.stack([batch_size]), minval=0, maxval=n_actions,
                                                       dtype=tf.int64)
                    chose_random = tf.random_uniform(tf.stack([batch_size]), minval=0, maxval=1,
                                                     dtype=tf.float32) < eps_ph
    ...
    def _setup_new_task(self, total_timesteps):
        ...
        items = [("observations0", self.observation_space.spaces['observation'].shape), \
                 ("actions", self.action_space.shape), \
                 ("rewards", (1,)), \
                 ("observations1", self.observation_space.spaces['observation'].shape), \
                 ("terminals1", (1,)), \
                 ("achieved_goal", env.observation_space.spaces['achieved_goal'].shape), \
                 ("desired_goal", env.observation_space.spaces['desired_goal'].shape)]
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        return super(GoalDQN, self).predict(observation, state=state, mask=mask, deterministic=deterministic)
