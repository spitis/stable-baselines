...
class DDPG(BaseRLModel):
    ...
    :param memory_policy: (ReplayBuffer) the replay buffer (if None, default to baselines.ddpg.memory.Memory)
    ...
    def __init__(self, policy, env, gamma=0.99, memory_policy=None, eval_env=None, nb_train_steps=50,
        ...
        if isinstance(policy, str) and not policy[:5] == "DDPG_":
            policy = "DDPG_" + policy
        ...
        self.gamma = gamma
        self.tau = tau
        self.memory_policy = memory_policy or ReplayBuffer
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.memory = self.memory_policy(self.buffer_size, items)
    ...
    def _train_step(self, step, writer, log=False):
        ...
        """
        # Get a batch
        obs0s, actions, rewards, obs1s, dones = self.memory.sample(batch_size=self.batch_size)

        if self.normalize_returns and self.enable_popart:
            old_mean, old_std, target_q = self.sess.run([self.ret_rms.mean, self.ret_rms.std, self.target_q],
                                                        feed_dict={
                                                            self.obs_target: obs1s,
                                                            self.rewards: rewards,
                                                            self.terminals1: dones.astype('float32')
                                                        })
    ...
    def _adapt_param_noise(self):
        ...
        distance = self.sess.run(self.adaptive_policy_distance, feed_dict={
            self.obs_adapt_noise: obs0s, self.obs_train: obs0s,
            self.param_noise_stddev: self.param_noise.current_stddev,
        })
                ...
                while True:
                    for _ in range(log_interval):
                        ...
                        for t_train in range(self.nb_train_steps):
                            ...
                            if self.memory.size >= self.batch_size and \
