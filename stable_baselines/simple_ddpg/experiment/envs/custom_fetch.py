...
class CustomFetchReachEnv(FetchReachEnv):
  ...
  def __init__(self, max_step=50, threshold_scale = 1.):
  ...
  def goal_state_compute_reward(self, achieved_goal, goal, info):
    # Compute distance between goal and the achieved goal.
    d = goal_distance(achieved_goal, goal)
    return (d <= self.distance_threshold * self.threshold_scale).astype(np.float32)
...
class CustomFetchPushEnv(FetchPushEnv):
  ...
  def __init__(self, max_step=50, reward_scale = 1., threshold_scale = 1.):
    super().__init__(reward_type='sparse')
    self.max_step = max_step
    self.num_step = 0
