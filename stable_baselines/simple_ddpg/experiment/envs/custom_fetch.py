...
class CustomFetchPushEnvMultiDimGoal(FetchPushEnv):

  def __init__(self, max_step=50, goal_dims=6):
    if goal_dims in [3,4,5,6,7,8,9]:
      self.goal_dims = goal_dims
    else:
      raise ValueError('goal_dims must be an integer from 3 to 9')
  ...
  def compute_reward(self, achieved_goal, goal, info):
    # Compute distance between goal and the achieved goal.
    if self.goal_dims == 3:
      return self.compute_reward_internal(achieved_goal, goal, info)
    
    coarse_d = goal_distance(achieved_goal[:3], goal[:3])
    fine_d = goal_distance(achieved_goal[3:], goal[3:])

    res =  ((coarse_d <= self.distance_threshold) and\
      (fine_d <= self.distance_threshold * np.sqrt((self.goal_dims - 3.) / 3.))).astype(np.float32)

    return res

  def compute_reward_internal(self, achieved_goal, goal, info):
    d = goal_distance(achieved_goal[:3], goal[:3])
    return (d <= self.distance_threshold).astype(np.float32)

  def goal_extraction_function(self, batched_states):
    return batched_states[:,[3,4,5,6,7,8,20,21,22][:self.goal_dims]]
  ...
  def _get_obs(self):
    ...
    achieved_goal = np.concatenate([np.squeeze(object_pos), np.squeeze(object_rel_pos), grip_velp])[:self.goal_dims]
  ...
  def _sample_goal(self):
    if self.has_object:
      ...
      goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-0.15, 0.15, size=3)
    return np.concatenate([goal, [0., 0., -0.05, 0., 0., 0.]])[:self.goal_dims]
  ...
  def step(self, action):
    ...
    reward = self.compute_reward_internal(obs['achieved_goal'], self.goal, None)
    done = 1. if self.num_step >= self.max_step else reward
    info = None
...
class CustomFetchSlideEnv_69(FetchSlideEnv):

  def __init__(self, six_or_nine=9, max_step=50, pos_threshold = 0.05, speed_threshold = 0.03):
    assert six_or_nine in [6, 9]
    self.mode = six_or_nine
    super().__init__(reward_type='sparse')
    self.max_step = max_step
    self.num_step = 0
    self.pos_threshold = pos_threshold
  ...
  def compute_reward(self, achieved_goal, goal, info):
    # Compute distance between goal and the achieved goal.
    if self.mode == 9:
      pos1 = goal_distance(achieved_goal[:3], goal[:3])
      pos2 = goal_distance(achieved_goal[3:6], goal[3:6])
      speed = goal_distance(achieved_goal[6:9], goal[6:9])
      return ((pos1 <= self.pos_threshold) and (pos2 <= self.pos_threshold) and (speed <= self.speed_threshold)).astype(np.float32)
    else:
      return self.compute_reward_internal(achieved_goal, goal, info)

  def compute_reward_internal(self, achieved_goal, goal, info):
    pos1 = goal_distance(achieved_goal[:3], goal[:3])
    speed = goal_distance(achieved_goal[-3:], goal[-3:])
    return ((pos1 <= self.pos_threshold) and (speed <= self.speed_threshold)).astype(np.float32)
